	...
	*/
	...
	else
	{
            /* Lets check for bogus names and clean them up
               we don't bother cleaning up ones from users, because
               we will never see them any more - Dianora */

		char clean_host[(2*HOSTLEN)+1];
		char *s, *d;
		int bogus_server = 0, found_dot = 0;
		...
		*d = '\0';
		...
		if (!found_dot || bogus_server)
		{
			sendto_ops("Bogus server name (%s) from %s",
				clean_host, get_client_name(cptr, TRUE));
			return 0;
		}
	}
...
#ifdef G_MODE
	sendto_flagops(UFLAGS_GMODE, "New Server: %s", host);
#endif
...
#define ByteString(x) (IsTera(x) ? "Tb " : \
			IsGig(x) ? "Gig" : \
			IsMeg(x) ? "Meg" : \
			"KB ")
#define ByteConvert(x) (IsTera(x) ? (float)((float)x/(float)Tera) : \
	...
	case '?':
	{
		...
		if (uptime)
		{
			sendto_one(sptr, ":%s %d %s :Since startup:",
				me.name, RPL_STATSDEBUG, parv[0]);
			temp = (float)((float)me.sendK/(float)uptime);
			sendto_one(sptr,
		":%s %d %s :Sent : %6.2f %s (%6.2f KB/s, %5.2f Mb/s)",
				me.name, RPL_STATSDEBUG, parv[0],
				ByteConvert(me.sendK), ByteString(me.sendK),
				temp, temp*(float)8/(float)1024);
			temp = (float)((float)me.receiveK/(float)uptime);
			sendto_one(sptr,
		":%s %d %s :Recv : %6.2f %s (%6.2f KB/s, %5.2f Mb/s)",
				me.name, RPL_STATSDEBUG, parv[0],
				ByteConvert(me.receiveK),
				ByteString(me.receiveK),
				temp, temp*(float)8/(float)1024);
		}
		if (tottime)
		{
			sendto_one(sptr, ":%s %d %s :Current Servers:",
				me.name, RPL_STATSDEBUG, parv[0]);
			temp = (float)((float)totsent/(float)tottime);
			sendto_one(sptr, ":%s %d %s :Sent : %6.2f %s (%6.2f KB/s, %5.2f Mb/s)",
				me.name, RPL_STATSDEBUG, parv[0],
				ByteConvert(totsent), ByteString(totsent),
				temp, temp*(float)8/(float)1024);
			temp = (float)((float)totrecv/(float)tottime);
			sendto_one(sptr, ":%s %d %s :Recv : %6.2f %s (%6.2f KB/s, %5.2f Mb/s)",
				me.name, RPL_STATSDEBUG, parv[0],
...
gohere:
...
#ifdef B_LINES
	case 'B' : case 'b' :
		report_dichconf_links(sptr, Blines, RPL_STATSBLINE, 'B',
			NULL, NULL, NULL);
	break;
#endif
	case 'C' : case 'c' :
...
#ifdef D_LINES
	case 'D' : case 'd' :
		report_dichconf_links(sptr, Dlines, RPL_STATSDLINE, 'D',
                        NULL, NULL, NULL);
	break;
#endif 
#ifdef E_LINES
	case 'E' : case 'e' :
		report_dichconf_links(sptr, Elines, RPL_STATSELINE, 'E',
                        NULL, NULL, NULL);
	break;
#endif 
#ifdef G_LINES
	case 'G' : case 'g' :
		report_dichconf_links(sptr, Glines, RPL_STATSGLINE, 'G',
			NULL, NULL, NULL);
		report_gline_rules(sptr);
	break;
#endif 
	case 'H' : case 'h' :
	...
	case 'I' : case 'i' :
#ifdef I_LINES
		report_dichconf_links(sptr, Ilines, RPL_STATSILINE, 'I',
			NULL, NULL, NULL);
#else
...
#ifdef J_LINES
	case 'J' : case 'j' :
		report_dichconf_links(sptr, Jlines, RPL_STATSJLINE, 'J',
                        NULL, NULL, NULL);
	break;
#endif 
	case 'K' : case 'k' :
...
#endif
		report_dichconf_links(sptr, Klines, RPL_STATSKLINE, 'K',
			user, host, &num);
#ifdef RESTRICT_STATSK
		if (!num && user)
	...
	case 'p' : case 'P':
			...
			i=new_operfdlist.entry[++j])
		{
			...
			totsent++;
			sendto_one(sptr,
				":%s %d %s :[%c] %s (%s@%s) idle %is",
				me.name, RPL_STATSDEBUG, sptr->name,
				IsOper(acptr) ? 'O' : 'o',
				acptr->name,
				acptr->user ? acptr->user->username : "<NULL>",
				acptr->user ? acptr->user->host : "<NULL>",
				acptr->user ? NOW-acptr->user->last : 0);
		}
		if (!totsent)
			sendto_one(sptr,
				":%s %d %s :No opers found",
				me.name, RPL_STATSDEBUG, sptr->name);
		break;
	case 'Q' : case 'q' :
...
char	*parv[];
{
	...
	if (!IsServer(sptr) && MyConnect(sptr))
		return m_operwall(cptr, sptr, parc, parv);
	if (BadPtr(message) || !IsServer(sptr))
		return 0;
...
char    *parv[];
{
	...
	*c15 = (char) 15;
	...
	if (!MyClient(sptr))
	{
		...
		if (IsPerson(sptr) && IsOper(sptr) && parv[1] &&
			*parv[1] && parv[2] && *parv[2] &&
			!rmatch(sptr->user->server, ALLOWED_KLINES))
			;
		else
#endif
			return 0;
	...
	if (test_kline_userhost(sptr, Klines, user, host))
		return 0;
#endif
	aconf = make_conf();
...
char	*parv[];
{
	...
	if (parv[1] && *parv[1] && !match(parv[1], "sdkjfkdsjf.aosidasdk.as02ks"))
		return 0; 
