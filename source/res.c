...
#ifndef lint
static  char rcsid[] = "@(#)$Id: res.c,v 1.1.1.1 1997/07/23 18:02:04 cbehrens Exp $";
#endif
...
#ifdef DONTNEEDTHIS
static  aCache  *rem_list PROTO((aCache *));
#endif
...
Reg1	ResRQ	*rptr;
{
	...
	(void)strncpy(hname, name, HOSTLEN);
	hname[HOSTLEN] = (char) 0;
	len = strlen(hname);

	if (len < HOSTLEN-2)
	if (rptr && !index(hname, '.') && _res.options & RES_DEFNAMES)
	{
		(void)strcat(hname, dot);
		len++;
		(void)strncat(hname, _res.defdname, HOSTLEN - len -1);
	}
...
HEADER	*hptr;
{
	...
	while (hptr->ancount-- > 0 && cp && cp < eob) {
		...
		if (!index(hostbuf,'.') && (_res.options & RES_DEFNAMES))
		{
			...
			hostbuf[sizeof(hostbuf)-1] = (char) 0;
...
char	*numb;
{
	...
	for (cp = cachetop; cp; cp = nextcp)
	{
		nextcp = cp->list_next;
		if (!cp->he.h_addr_list) /* should have been caught above */
		{
#ifdef DONTNEEDTHIS
			cp = rem_list(cp);
#else
